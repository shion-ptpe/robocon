...
int main(){
    ...
    while(1){
        ...
        if(0<Distance  && CHECK==SUM){ // Strength<100かDistance<0か
            sprintf(data,"Distance(m):%f",Distance/100);
            read(data);
            sprintf(data,"Strength:%f",Strength);
            read(data);
            sprintf(data,"Temperature:%f",Temperature/8256);
            read(data);
            __delay_ms(1000)
            if(30<Distance<200){
                music();
...
//            if((0<Distance<100)  && (CHECK==SUM)){ // Strength<100かDistance<0か
//                sprintf(data,"Distance(m):%f",Distance/100);
//                read(data);
//                sprintf(data,"Strength:%f",Strength);
//                read(data);
...
void music(){
    int i=0;
    double ans;
    double clock=FCY;  // FCY==4000000
//    int melody[]={261, 293, 329, 349, 391,440,494,523};
//    int melody[]={C4, D4, E4, F4, G4,A4,B4,C5};
    int melody[]={0,329, 261, 195, 261, 293, 783, 0, 195, 293, 329, 293, 195, 261, 261};  // ファミマの入店音?

//    int melody[]={E4, C4, G3, C4, D4, G5, 0, G3, D4, E4, D4, G3, C4, C4};  // ファミマの入店音?
...
// PWMのOC1
   RPOR4bits.RP8R = 18;   //  ピン…RPn(n=7)、OC1モジュール(0d18)  // RP4使えなかった
